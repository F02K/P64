...
UI_FRAGMENT_SHADER = SPRITE_FRAGMENT_SHADER

PARTICLE_VERTEX_SHADER = SPRITE_VERTEX_SHADER
...
void main() {
    vec4 texel = texture(u_texture, v_uv);
    float alpha = texel.a * v_color.r * clamp(u_alpha, 0.0, 1.0);
    if (alpha < u_alpha_cutoff) {
        discard;
    }
    fragColor = vec4(texel.rgb * u_base_color, alpha);
}
"""
