...
from p64.engine.components import Canvas, MeshRenderer, ModelRenderer, RectTransform
...
def virtual_submesh_labels(entity: Entity, metadata_for_mesh: Callable[[str], AssetMetadata | None]) -> list[str]:
    ...
    for component in entity.components:
        if isinstance(component, ModelRenderer) and component.model:
            metadata = metadata_for_mesh(component.model)
            if metadata is not None:
                meshes = model_meshes(metadata)
                labels.append(f"Model: {metadata.source or metadata.id} ({len(meshes)} mesh(es))")
            continue
