Shader "P64Builtin/Standard VertexLit"
{
    Vertex
    {
        ...
        void main() {
            ...
            for (int i = 0; i < min(u_light_count, P64_MAX_LIGHTS); i++) {
                vec3 light_vec = normalize(-u_light_direction[i]);
                float attenuation = 1.0;
                if (u_light_kind[i] != 0) {
                    vec3 to_light = u_light_position[i] - world_pos.xyz;
                    float distance_to_light = length(to_light);
                    light_vec = normalize(to_light);
                    attenuation = pow(clamp(1.0 - distance_to_light / max(u_light_range[i], 0.001), 0.0, 1.0), max(u_light_falloff[i], 0.001));
                    if (u_light_kind[i] == 2) {
                        vec3 from_light = normalize(-to_light);
                        vec3 spot_dir = normalize(u_light_direction[i]);
                        float cutoff = cos(radians(clamp(u_light_spot_angle[i], 1.0, 179.0)) * 0.5);
                        float inner = cos(radians(clamp(u_light_spot_angle[i] * 0.75, 1.0, 179.0)) * 0.5);
                        attenuation *= smoothstep(cutoff, inner, dot(from_light, spot_dir));
                    }
                }
                light += u_light_color[i] * u_light_intensity[i] * max(dot(normal, light_vec), 0.0) * attenuation;
            }
    ...
    Fragment
    {
        ...
        void main() {
            ...
            vec3 quantized = floor(texel.rgb * v_light * u_color_levels) / u_color_levels;
